To troubleshoot the container initialization, such as when server files are pre-downloaded, set the environment variable `DEBUG` to `true`. The container logs will include **much more** output, and it is highly recommended including that output when reporting any [issues](https://github.com/energypatrikhu/pterodactyl-minecraft-server/issues).
...
```json
{
  ...
  "org.opencontainers.image.source": "https://github.com/energypatrikhu/pterodactyl-minecraft-server",
  "org.opencontainers.image.title": "docker-minecraft-server",
  "org.opencontainers.image.url": "https://github.com/energypatrikhu/pterodactyl-minecraft-server",
...
- `org.opencontainers.image.revision` : which maps to <https://github.com/energypatrikhu/pterodactyl-minecraft-server/commit/REVISION>
